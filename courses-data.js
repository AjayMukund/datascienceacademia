const DSA_COURSES = {
  ...
  'python': {
    ...
    modules: [
      { week: 1, title: 'Python Fundamentals', topics: ['Installing Python and setting up VS Code / Jupyter', 'Variables, data types (int, float, str, bool)', 'Arithmetic, comparison and logical operators', 'String methods and f-string formatting', 'User input and print formatting', 'Type casting and basic I/O programs'] },
      { week: 2, title: 'Control Flow', topics: ['if, elif and else statements', 'for loops and while loops', 'break, continue and pass keywords', 'Nested loops and loop patterns', 'List comprehensions and generator expressions', 'Practice: number patterns and simple games'] },
      { week: 3, title: 'Functions & Modules', topics: ['Defining and calling functions', 'Positional, keyword, *args and **kwargs', 'Default parameter values and return statements', 'Lambda functions and higher-order functions (map, filter)', 'Scope: local vs global variables', 'Importing and using built-in modules (math, random, datetime)'] },
      { week: 4, title: 'Data Structures', topics: ['Lists — indexing, slicing, methods', 'Tuples — immutability and packing/unpacking', 'Dictionaries — CRUD operations, nested dicts', 'Sets — union, intersection, difference', 'Choosing the right data structure', 'Mini-project: student grade tracker'] },
      { week: 5, title: 'Object-Oriented Programming', topics: ['Classes and objects — attributes and methods', 'The __init__ constructor and self', 'Inheritance and method overriding', 'Encapsulation and name mangling', 'Polymorphism and duck typing', 'Practice: building a library management system'] },
      { week: 6, title: 'File Handling & Exception Handling', topics: ['Reading and writing text files (open, read, write, append)', 'Working with CSV files using the csv module', 'Parsing JSON data with the json module', 'try, except, finally and raise', 'Custom exception classes', 'Context managers with the with statement'] },
      { week: 7, title: 'Scientific Python — NumPy & Pandas', topics: ['NumPy arrays — creation, indexing, slicing', 'Array operations, broadcasting and vectorisation', 'Pandas Series and DataFrames', 'Loading data from CSV and Excel', 'Data selection: loc, iloc, boolean indexing', 'Aggregation: groupby, pivot_table, describe'] },
      { week: 8, title: 'Capstone Project & Matplotlib Basics', topics: ['Introduction to Matplotlib — line, bar and scatter plots', 'Plot customisation — labels, titles, legends, colour', 'Exploratory Data Analysis (EDA) workflow', 'Capstone: end-to-end data pipeline — load, clean, analyse and visualise a real-world dataset', 'Code review and best practices', 'Next steps: roadmap into Machine Learning'] }
    ]
  },
  ...
  'sql': {
    ...
    modules: [
      { week: 1, title: 'Database Fundamentals & Basic Queries', topics: ['What is a relational database? Tables, rows and columns', 'Installing PostgreSQL and connecting via DBeaver', 'SELECT, FROM and the query execution order', 'WHERE clause — filtering with comparison and logical operators', 'ORDER BY, LIMIT and OFFSET', 'Working with NULL values — IS NULL, COALESCE, NULLIF'] },
      { week: 2, title: 'Aggregations & Grouping', topics: ['Aggregate functions: COUNT, SUM, AVG, MIN, MAX', 'GROUP BY — grouping rows and computing summaries', 'HAVING clause — filtering aggregated results', 'DISTINCT — removing duplicates', 'Rounding, casting and data type conversions', 'Practice: business KPI reporting queries'] },
      { week: 3, title: 'Joins — Combining Tables', topics: ['Primary keys, foreign keys and referential integrity', 'INNER JOIN — matching rows in both tables', 'LEFT and RIGHT OUTER JOINs', 'FULL OUTER JOIN and CROSS JOIN', 'Self joins — joining a table to itself', 'Multi-table joins and joining on multiple conditions'] },
      { week: 4, title: 'Subqueries, CTEs & Window Functions', topics: ['Scalar, row and table subqueries', 'Correlated subqueries and when to use them', 'Common Table Expressions (CTEs) with WITH clause', 'Recursive CTEs for hierarchical data', 'Window functions: ROW_NUMBER, RANK, DENSE_RANK', 'LAG, LEAD, FIRST_VALUE, LAST_VALUE, running totals'] },
      { week: 5, title: 'Query Optimisation & Capstone', topics: ['Indexes — B-tree, hash; when and how to create them', 'EXPLAIN and EXPLAIN ANALYSE — reading query plans', 'Query rewriting techniques for performance', 'Views and materialised views', 'SQL from Python using psycopg2 and SQLAlchemy', 'Capstone: end-to-end analytics report on a real e-commerce dataset'] }
    ]
  },
  ...
  'r-programming': {
    ...
    modules: [
      ...
      { week: 2, title: 'R Data Structures', topics: ['Matrices — creation, row/column operations', 'Lists — heterogeneous containers, nested structures', 'Data frames — the core tabular structure in R', 'Tibbles — modern data frames from the tibble package', 'Importing data: read.csv, read_csv, readxl, readRDS', 'Inspecting data: str, summary, head, tail, dim'] },
      { week: 3, title: 'Data Wrangling with dplyr & tidyr', topics: ['The pipe operator — %>% and |>', 'filter() — selecting rows by condition', 'select(), rename() and relocate() — column management', 'mutate() — creating and transforming columns', 'group_by() and summarise() — aggregation workflows', 'pivot_longer() and pivot_wider() — reshaping data; join functions'] },
